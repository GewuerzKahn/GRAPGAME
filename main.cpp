...
void initObjects(){
	...
	terrain->setMaterial(EnumMaterial::BRASS);
...
void render2(void) {
	...
	if (PreviousClock / 1000 > 60){
	char endgame[50];
	sprintf(endgame, "GAME OVER", "");
	renderBitmapString(10, 32, 0, GLUT_BITMAP_TIMES_ROMAN_24, endgame);
			...
			if (!bullets[x]->isDead()){
				...
				for (int y = 0; y < targets.size(); y++){
					...
					if (!targets[y]->isDead()){
						...
						if (checkCollision(bullets[x], targets[y])){
							//Score::addScore(1);
							cout << "true" << "\n";
							bullets[x]->setDeadState(true);
							targets[y]->setDeadState(true);
						}
					}
				}
			}
...
bool checkCollision(OwnObject *a, OwnObject *b){
	{
		...
		//check the X axis
	//	cout << bX << " " << bY << " " << bZ << "\n";
	//	cout << a->getSize().x << " " << b->getSize().x << "\n";

		if (abs(a->getPosition().x-bX) < (a->getSize().x + b->getSize().x))
		{
			//check the Y axis
			if (abs(a->getPosition().y-bY) < a->getSize().y + b->getSize().y)
			{
				//check the Z axis
			if (abs(a->getPosition().z-bZ) < (a->getSize().z + b->getSize().z))
				{
					return true;
				}
			}
