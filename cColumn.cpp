...
bool cColumn::InsideGatheringArea(float posx, float posy, float posz)
{
	if(yaw == -90)
	{
		if(posz <= z + GATHERNG_AREA_SIDE/2 && posz >= z - GATHERNG_AREA_SIDE/2 && posx <= x && posx >= x - GATHERNG_AREA_SIDE) return true;
		else return false;
	}
	if(yaw == 90)
	{
		if(posz <= z + GATHERNG_AREA_SIDE/2 && posz >= z - GATHERNG_AREA_SIDE/2 && posx <= x + GATHERNG_AREA_SIDE && posx >= x) return true;
		else return false;
	}
	if(yaw == 180)
	{
		if(posz <= z && posz >= z - GATHERNG_AREA_SIDE && posx <= x + GATHERNG_AREA_SIDE/2 && posx >= x - GATHERNG_AREA_SIDE/2) return true;
		else return false;
	}
}
